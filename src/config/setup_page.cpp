...
std::string buildSetupPageHtml(
  ...
  html << "<script>const ssid=document.getElementById('wifi_ssid');const auto=document.getElementById('auto_rtc');function sync(){auto.disabled=ssid.value.trim()==='';if(auto.disabled)auto.checked=false;}function pickSsid(v){ssid.value=v;sync();}document.querySelectorAll('.wifi button[data-ssid]').forEach(b=>b.addEventListener('click',()=>pickSsid(b.dataset.ssid)));ssid.addEventListener('input',sync);sync();document.getElementById('osm_link').addEventListener('input',function(){var val=this.value;var m=val.match(/#map=[0-9.]+\\/([0-9.-]+)\\/([0-9.-]+)/);if(m){document.getElementById('latitude').value=m[1];document.getElementById('longitude').value=m[2];}else{var latMatch=val.match(/[?&](?:mlat|lat)=([0-9.-]+)/);var lonMatch=val.match(/[?&](?:mlon|lon)=([0-9.-]+)/);if(latMatch&&lonMatch){document.getElementById('latitude').value=latMatch[1];document.getElementById('longitude').value=lonMatch[1];}}});</script>";
  html << "</body></html>";
  return html.str();
}
